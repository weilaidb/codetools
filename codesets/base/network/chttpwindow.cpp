...
CHttpWindow::CHttpWindow(QWidget *parent) :
    QMainWindow(parent),
...
CHttpWindow::~CHttpWindow()
{
    delete ui;
}

void CHttpWindow::on_finished()
{//网络响应结束
    QFileInfo fileInfo;
...
void CHttpWindow::on_readyRead()
{//读取下载的数据
    downloadedFile->write(reply->readAll());
}

void CHttpWindow::on_downloadProgress(qint64 bytesRead, qint64 totalBytes)
{//下载进程
    ui->progressBar->setMaximum(totalBytes);
    ui->progressBar->setValue(bytesRead);
}

void CHttpWindow::on_btnDefaultPath_clicked()
{//缺省路径  按钮
...
void CHttpWindow::on_btnDownload_clicked()
{//开始下载 按钮
...
void CHttpWindow::on_editURL_textChanged(const QString &arg1)
{
    Q_UNUSED(arg1);
